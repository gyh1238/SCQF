...
def draw(output_dir: Path) -> tuple[Path, Path]:
    ...
    ax.plot(
        ...
        linewidth=2.5,
        linestyle=(0, (7, 3)),
        label=r"Proposed enhanced ($b_m=3$, $s=4$)",
        zorder=4,
    )
    ax.plot(
        n_values,
    ...
    ax.plot(
        ...
        linewidth=2.9,
        label=r"Classical: $0.400N_z^3$ ns",
        zorder=2,
    )
    ...
    fig.tight_layout(rect=(0.015, 0.025, 0.995, 0.98))
