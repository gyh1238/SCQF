...
The target common exponent is $\beta=1.5$, so $\lambda=\beta/\bar{u}$ with $\bar{u}$ taken as the mean over UEs of the best utility their candidates offer. Each zone applies $k=1$ round in \eqref{eq:circuit-chain} and reports $K_z=200$ accepted joint samples, with re-sampling triggered when $\widetilde{K}_z<K_{\min}=25$. A report costs about $K_z/P_z(k)$ executions, with $P_z(k)\simeq(2k+1)^2\mu_z$ here, so one round gives up to a ninefold reduction and about sixfold as measured. The round count maximizing $P_z(k)$ ranges from none for the sparsest zones to about $890$ for the most tightly constrained, motivating the practical choice $k=1$.
...
The coupling map is a gate count rather than a simulation: transpiled to the basis gates of a calibrated $127$-qubit backend and routed onto its heavy-hex lattice, a four-UE zone circuit grows from $384$ to $969$ two-qubit gates and its depth from $773$ to $2\,717$. Under a device noise model at that backend's median error rates, simulated at all-to-all connectivity with $10^4$ shots per zone, the accepted-branch mass falls to $18\%$ of its noise-free value and the distance from $p_z$ is $0.134$, against $0.005$ noise-free at the same shot count; both trace to the depth of the QFT accumulators and the superflag conjunction. Sweeping the two-qubit error rate places the device between them, $10^{-3}$ still holding the distance at $0.016$ and $77\%$ of the accepted mass.

Strict feasibility survives unchanged. The acceptance test of Section~\ref{sec:distribution} applies to the decoded assignment rather than the internal flags, so corrupted outcomes are discarded and no infeasible assignment entered any zone report: the discard rate rises from $33\%$ to $88\%$, and the share of shots reaching that test only to decode infeasible rises from $5.0\%$ to $12.3\%$. Neither share is zero without noise, since an infeasible assignment can leave every utility qubit in $\ket{0}$. 

Since \eqref{eq:op-prep} is a product over UEs before screening and $k$ enters only through $P_z(k)$, independent per-UE utility-weighted sampling followed by feasibility screening induces the same accepted-branch law; the experiments below use that classical sampler and account execution cost separately.

\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth]{fig/fig_snapshot}
    \caption{One partitioned region: the zones a $2000$-gate budget produces, four accepted laws against their exact references, the belief a product of marginals misplaces on the two most dependent boundary pairs, and the order decimation commits in.}
    \label{fig:snapshot}
\end{figure*}
...
\begin{figure}[!t]
    ...
    \caption{Circuit cost, coordination traffic, and utility as the region grows: the per-zone quantities stay bounded while the centralized circuit does not.}
