...
\end{align}
where $\lambda>0$ controls how strongly utility differences are reflected in the weights. This mapping gives $g_{i,j}=1$ for a
...
% VERIFY: the repository URL below currently returns 404 to an unauthenticated client.  Make it public before submission, or the artifact claim cannot be checked by a reviewer.
...
The evaluation instantiates \eqref{eq:general-assignment} for joint AP--RB assignment. Each candidate $j=(a,r)\in\mathcal{C}_i$ loads two resources at once: the admission budget of AP $a$, with $c_{m,i,j}=d_i$ and $W_m=W_a$, and the occupancy of RB $r$, with $c_{m,i,j}=1$ and $W_m=W_r$. The oracle of Section~\ref{sec:oracle} evaluates both on the same assignment.

APs sit on a jittered unit grid of side $g$, measured in AP spacings, with two UEs and four RBs per AP, a coverage radius of $1.2$ spacings, and at most two joint candidates per UE, so $|\mathcal{C}_i|\leq2$ and $\ell_i=1$. The limits are $W_r=1$, $W_a=4$, and $d_i=1$, and utilities decay with UE--AP distance with a per-RB perturbation. Only $g$ varies, holding density, candidate degree, and capacity profile fixed. Greedy region growing with single-AP boundary refinement partitions the AP co-coverage graph, minimizing $|\mathcal{B}|$ subject to the partition budget $G_z\leq2000$ for one oracle $\mathcal{O}_z$.

The target common exponent is $\beta=1.5$, so $\lambda=\beta/\bar{u}$ with $\bar{u}$ the mean candidate utility. Each zone applies $k=1$ round in \eqref{eq:circuit-chain} and reports $K_z=200$ accepted joint samples, with re-sampling triggered when $\widetilde{K}_z<K_{\min}=25$. A report costs about $K_z/P_z(k)$ executions, with $P_z(k)\simeq(2k+1)^2\mu_z$ here, so one round gives roughly a ninefold reduction. The round count maximizing $P_z(k)$ ranges from about five for the sparsest zones to about $900$ for the most tightly constrained, motivating the practical choice $k=1$.

% VERIFY(k=1): beta_z changes when k moves from 0 to 1, so re-read the
% corresponding mu_z values before keeping "about five" and "about 900".

To respect a per-zone budget of $10^4$ executions, zone $z$ runs at the largest exponent $\beta_z\leq\beta$ that meets the budget at $k=1$, and its samples are reconstructed at the common exponent using the reweighting in Appendix~\ref{app:reweighting}. Across zones, $\beta_z$ ranges from $0.31$ to $1.50$ and $\widetilde{K}_z$ stays at or above $118$ of $200$ retained samples.

% VERIFY(k=1): 0.31 and 118. The k=0 draft had 0.08--1.50 and 73.

\subsection{Zone-local distributions, exactly and under noise}
\label{subsec:eval_local_law}

% VERIFY: 2.7e-15 asserts that the implemented rotation follows (19)-(20) exactly.  The rotation code released for [jang2025cqf] instead uses  w_norm = w / max_w ;  theta = arccos(sqrt(w_norm))  -- a LINEAR weight with a MISSING factor of 2 relative to (20).  On a two-UE example that weight leaves the ranking intact but its ratio to exp[lambda*J] varies by a factor of 10.5 across assignments, so it does not satisfy (15) and would not give a TVD anywhere near 1e-15.  Confirm the SCQF rotation code against (19)-(20) before submission; this number and the Appendix A-B ordering claim both depend on it.
Six zones of at most seven UEs and 24 qubits admit statevector checks. The largest total-variation distance from $p_z(\bm z)\propto\exp[\lambda J_z(\bm z)]$ on $\mathcal{F}_z$ is $2.7\times10^{-15}$ with no accepted probability outside $\mathcal{F}_z$, so the rotations realize the weight \eqref{eq:composability} requires and the superflag admits exactly the feasible set.

% VERIFY(k=1): 52%, 0.094, 0.061.  Report the two TVDs as measured; they are two separate measurements at equal sample size, NOT an additive decomposition of the noisy figure.  0.061 must be the same run quoted in Sec. V-C.
Under a device noise model, transpiled to its basis gates and coupling map with $10^4$ shots per zone, the accepted-branch mass falls to $52\%$ of its noise-free value and the distance from $p_z$ is $0.094$, against $0.061$ noise-free at the same $K_z=200$; both trace to the transpiled depth of the QFT accumulators and the superflag conjunction.

% VERIFY(k=1): 12% and 29%.  The noise-free rate is not zero: an infeasible assignment can still show all utility qubits in |0>.
Strict feasibility survives unchanged. The acceptance test of Section~\ref{sec:distribution} applies to the decoded assignment rather than the internal flags, so corrupted outcomes are discarded: the discard rate rises from $12\%$ to $29\%$, and no infeasible assignment entered any zone report. 
...
% ---------------------------------------------------------------
%  FIGURE 8 -- two-column float, declared at the end of Sec. V-B so
%  that [!t] can place it atop the page on which Sec. V-C begins.
% ---------------------------------------------------------------
\begin{figure*}[!t]
    \centering
...
% CHECKED: 16 seeds -> 14 feasible -> statistics over all 14, of which re-conditioning wins on 13.  Consistent with the panel annotation "+1.2 pp on average, 13 of 14 cases".  No 13-feasible-seed reading remains anywhere in the section.
% VERIFY(k=1): 0.061 -- must be the same run quoted in Sec. V-B.
Fig.~\ref{fig:snapshot} follows one campus instance of $M=25$ APs and $N=50$ UEs. It is drawn from a family of 16 seeds on the same map, 14 of which are globally feasible, and the statistics below are over those 14. Throughout, $J^{\star}$ is the centralized strict optimum from HiGHS. The partition gives $|\mathcal{Z}|=14$ zones within budget and $|\mathcal{B}|=23$ boundary UEs, so $46\%$ of the UEs require coordination, and the four displayed zone distributions match their exact accepted laws to $0.061$ with no mass outside $\mathcal{F}_z$ despite differing $\beta_z$.

% VERIFY(k=1): 19% and 23%; read both off the rendered panels.  18% = extra executions from the 5 re-samples / total executions for the region, using the pinned-preparation acceptance mass rather than the original mu_z.
On the two most dependent boundary-UE pairs the product of the marginals places $19\%$ and $23\%$ of its belief on joint choices that violate a capacity condition in $\mathcal{M}_z$ and that no sample in $\mathcal{S}_z$ realizes. Against a one-pass baseline identical in sampling and commitment rule, re-conditioning wins on 13 of the 14 instances by $1.2$ percentage points of $J^{\star}$. Five re-samples add $18\%$ to the region's execution count. The result satisfies every capacity and consistency condition and reaches $99.2\%$ of $J^{\star}$, the family mean, inside the observed $98.1$--$100\%$ range.

% ---------------------------------------------------------------
%  FIGURE 9 -- single column, top of the page carrying Sec. V-D.
%  ARTWORK: rename the y-axis to "Modeled logical latency (s)" so that
%  the axis, the caption, and the body all use one name for tau_z.
% ---------------------------------------------------------------
\begin{figure}[!t]
    \centering
...
For zone $z$, let $D_z=\sum_{i\in\mathcal{U}_z}|\mathcal{C}_i|$ denote the number of evaluated candidate edges, $\ell_{\max,z}=\max_{i\in\mathcal{U}_z}\lceil\log_2|\mathcal{C}_i|\rceil$ the widest candidate code, and $\ell_{\mathrm{cnt},z}$ the load-counter width. With approximate-QFT arithmetic and standard multi-control decompositions, the modeled depth $T_z$ and two-qubit count $G_z$ of one oracle $\mathcal{O}_z$ scale as
\begin{equation}
    T_z,\;G_z=O\!\left(
    D_z\left(\ell_{\max,z}+\ell_{\mathrm{cnt},z}\right)
    +|\mathcal{M}_z|\,\ell_{\mathrm{cnt},z}
    \right).
...
For zone $z$ with $n_z^{\mathrm{re}}$ re-samples, coordination traffic is modeled as $(1+n_z^{\mathrm{re}})K_z\bigl(\sum_{i\in\mathcal{B}_z}\lceil\log_2|\mathcal{C}_i|\rceil+32\bigr)$ bits, where the 32-bit field carries $J_z$ for the reconstruction of Appendix~\ref{app:reweighting}; the plotted values are converted to kilobytes. Regional traffic grows by $8.8\times$ as the zone count grows by $8.4\times$, so the mean report per zone increases by only $5\%$ and the per-zone load remains nearly unchanged as the region expands.

Coordinated utility stays between $98.81\%$ and $99.87\%$ of $J^{\star}$ with no downward trend, and all 40 runs finish globally feasible, so \eqref{eq:factorization} holds. Decomposition, finite sampling, and commitment cost about one percentage point, which does not accumulate over the evaluated range.

% VERIFY(k=1): every figure in the paragraph below. They are the k=0 values
% (10^6--10^8, 2.6e8, 26,000) divided by nine. Regenerate rather than rescale:
% beta_z shifts with k, so the two utility percentages move too.
The exponent backoff keeps sampling affordable. On a separate 16-instance comparison, the most tightly constrained zones need $10^5$ to $3\times10^7$ executions at the target exponent with $k=1$; capping each zone at $10^4$ cuts the peak single-zone requirement from $2.9\times10^7$ to $10^4$, a factor of about $2\,900$, while mean utility moves from $99.31\%$ to $99.28\%$ of $J^{\star}$, a loss of $0.03$ percentage points on average and $0.55$ at worst. Reconstruction is unbiased, so the cap trades effective sample size for execution cost rather than systematic bias.
}
